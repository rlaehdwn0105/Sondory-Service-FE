...
const RecentSongs = () => {
  const { userRecentSongs, fetchRecentsSong, error } = useMusicStore();
  ...
  if (error) return <p className="text-red-500 text-lg px-6">{error}</p>;

  return (
    <div className="relative mb-8">
      <div className="flex items-center justify-between px-4 mb-4">
        <h2 className="text-2xl font-bold text-white">Recently Played Songs</h2>
        <Link
          to="/recent"
      ...
      >
        {userRecentSongs.slice(0, 20).map((song) => {
